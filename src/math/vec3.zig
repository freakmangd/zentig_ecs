...
pub const Vec3 = extern struct {
    ...
    /// Returns a new Vec3 with all of its components set to a number within [min, max)
    ...
    pub fn format(value: Vec3, writer: *std.Io.Writer) !void {
        try writer.print("Vec3({}, {}, {})", .{ value.x, value.y, value.z });
    }

    pub fn formatNumber(value: Vec3, writer: *std.Io.Writer, options: std.fmt.Number) !void {
        try writer.writeAll("(");
